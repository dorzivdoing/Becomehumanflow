      ...
      <div style={{ background: C.bg, padding: '120px 40px 80px', direction: 'rtl', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: '760px', margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: '40px', alignItems: 'flex-start', flexWrap: 'wrap', marginBottom: '40px' }}>
            <div style={{ width: '150px', height: '190px', borderRadius: '12px', flexShrink: 0, border: `2px solid ${C.sage}`, overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, minWidth: '240px' }}>
              <p style={{ fontSize: '16px', color: C.textMid, lineHeight: 1.9, margin: 0, fontFamily: "'Assistant', sans-serif" }}>{paragraphs[0]}</p>
            </div>
          </div>
          {paragraphs.slice(1).map((p, i) => (
            <p key={i} style={{ fontSize: '16px', color: C.textMid, lineHeight: 1.9, margin: '0 0 24px', fontFamily: "'Assistant', sans-serif" }}>{p}</p>
          ))}
        </div>
        <QuoteBlock
